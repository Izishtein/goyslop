...
Разобрано 2026-09-08 по первоисточнику — Core Rulebook I, pp. 192–202 (§ 3 плана, «по запросу»), реализовано на листе.

**Это не боевой компаньон, а система публикации своего PC как гостевого NPC для чужой группы** — то, что раньше подписывалось как «отдельный лист» на деле оказалось: игрок конвертирует уже существующего персонажа в упрощённый «формат Fellow» и выкладывает его в соцсетях/фан-журналах, чтобы другая группа могла одолжить его на приключение. Fellow неуязвим (не получает урона и негативных эффектов, при разгроме партии автоматически возвращается в безопасное место), управляется игроками совместно и действует только по своей «Fellow Action Table».

**Формат данных Fellow (пример из книги, Wolfe):**

```
Name: Wolfe
Race: Human   Gender: Male   Age: 16
Adventurer Level: 2   MP: 10
Classes: Fighter 2, Scout 1
Languages: Trade Common (Speak/Read), Regional (Burlight: Speak/Read)
Self-Introduction: "I'm a hot-blooded guy who loves a good adventure. Nice to meet you!"
Experience: Yes/No   Reward: Yes/No

Fellow Action Table
1d    Action                Dialogue                          Value   Effect
1-2   Attack with Sword     "I'll turn you to rust on my       12      Power 25/Crit Value 10+4
      (Melee Attack)         blade."
3-4   Observation Check     "What's this?!"                    11
      (Scout)
5     [Power Strike]        "This is the end!"                 14      Power 25/Crit Value 10+8,
      (Melee Attack)                                                    spend next turn recovering
6     Scout Movement Check  "Leave this one to me!"             13
```

**Верхняя половина** — данные персонажа, скопированные с его листа как есть (имя, раса, пол, возраст, уровень авантюриста, MP — не HP, языки), плюс самопрезентация и два флажка «хочет ли опыт/награду».

**Нижняя половина** — Fellow Action Table: бросок 1d определяет, какое из перечисленных действий Fellow пытается сделать в этот ход; цель действия выбирается уже после того, как определилось действие и его Value (в отличие от обычного PC). Таблица собирается вручную под конкретного персонажа из его реальных способностей — общего каталога действий нет и не может быть.

**Реализация на листе (`FellowSection.tsx`, опциональная секция в конце листа):** раса/пол/возраст/уровень/MP/классы берутся с листа без дублирования в схеме; в схему (`FellowSchema`) добавлены только самопрезентация, языки (свободным текстом — общего трекера языков на листе нет и не было), два флажка опыт/награда (как `<select>` Yes/No, не чекбоксы — печать снимает оформление с чекбокса) и сама Action Table свободным текстом (роль/действие/реплика/значение/эффект).
